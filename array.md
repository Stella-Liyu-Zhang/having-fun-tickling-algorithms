...
Arrays.sort(a1);
````
An array has a fixed capacity, 我们必须specify the size of the array when we initialize the array. 
Therefore, we could use built in 的 dynamic array，是一个random access list data structure, but with variable size. 
For example, we have 
## ArrayList in Java.
